<template>
  ...
  >
    ...
    >
      ...
      <!-- Anthropic API Key: Inject X-Origin-Model-Id header -->
      <div
        v-if="account?.platform === 'anthropic' && account?.type === 'apikey'"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.anthropic.injectOriginModelId') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.anthropic.injectOriginModelIdDesc') }}
            </p>
          </div>
          <button
            type="button"
            @click="injectOriginModelIdHeader = !injectOriginModelIdHeader"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              injectOriginModelIdHeader ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                injectOriginModelIdHeader ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
const injectOriginModelIdHeader = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  injectOriginModelIdHeader.value = false
  ...
  if (newAccount.platform === 'anthropic' && newAccount.type === 'apikey') {
    ...
    injectOriginModelIdHeader.value = extra?.inject_origin_model_id_header === true
  ...
  try {
    ...
    if ((props.account.platform === 'openai' || props.account.platform === 'grok') && props.account.type === 'oauth') {
      ...
      if (props.account.platform === 'openai') {
        ...
        // Grok OAuth: 仅持久化模型限制映射（无透传/compact 概念）
        const modelMapping = buildModelRestrictionMapping()
        if (modelMapping) {
          newCredentials.model_mapping = modelMapping
        } else {
    ...
    if (props.account.platform === 'anthropic' && props.account.type === 'apikey') {
      ...
      if (injectOriginModelIdHeader.value) {
        newExtra.inject_origin_model_id_header = true
      } else {
        delete newExtra.inject_origin_model_id_header
      }
