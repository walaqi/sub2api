export default {
  ...
  auth: {
    ...
    emailSuffixAllowedHint: '请使用以下邮箱域名注册：{suffixes}',
  ...
  usage: {
    ...
    giftDeducted: '赠金扣减',
    giftCostLabel: '赠金',
    rechargeDeducted: '充值扣减',
    rechargeCostLabel: '余额',
  ...
  affiliate: {
    ...
    superReferral: {
      ...
      rechargeNeededAny: '完成一次充值即可成为超级邀请',
  ...
  profile: {
    ...
    giftExpiringAt: '{date} 过期',
    giftModePriority: '优先扣除',
    giftModeRatio: '比例',
  ...
  admin: {
    ...
    users: {
      ...
      typeSubscription: '订阅',
      giftStatusRevoked: '已撤销',
      giftStatusExpired: '已过期',
      giftStatusExhausted: '已耗尽',
    ...
    groups: {
      model5hLimits: {
        ...
        hint: 'JSON 对象，key 为精确模型名，value 为该模型在任意 5 小时窗口内的 USD 上限（示例见下方输入框占位提示）。对每个用户单独计量，且对所有用户（含订阅用户）生效。留空表示不限制。',
    ...
    accounts: {
      ...
      mapRequestModels: '以 JSON 格式填写模型映射：{\'{"请求模型": "实际上游模型"}\'}。键是客户端请求的模型，值是转发到上游的实际模型。',
      ...
      modelMappingJsonInvalid: 'JSON 格式无效',
      modelMappingJsonNotObject: '必须是一个 JSON 对象',
      modelMappingJsonValueNotString: '所有的值必须是字符串。键 "{key}" 的值不是字符串。',
      modelMappingJsonKeyWildcard: '键 "{key}" 的通配符 * 只能放在末尾',
      modelMappingJsonValueWildcard: '键 "{key}" 的目标模型不能包含通配符 *',
    ...
    announcements: {
      ...
      form: {
        ...
        referralInviterRewardBlocked: '有待领取的达标奖励（配额已用尽）'
      },
      operators: {
        gt: '>',
        gte: '≥',
    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          referralRewardEnabled: '启用超级邀请奖励',
          referralRewardEnabledHint: '开启后新用户注册即获赠金，被邀请人消费达标后邀请人获赠金奖励。',
          referralInviteeAmount: '被邀请人注册赠金',
          referralInviteeAmountHint: '新用户通过邀请链接注册后立即获得的赠金金额（USD）。',
          ...
          referralEligibilityGrantMode: '资格获得方式',
          referralEligibilityGrantModeHint: '控制邀请人何时获得超级邀请资格。修改仅影响后续绑定/懒补建。',
          referralEligibilityGrantModeBindKeyClaim: '领取 Key 后立即获得',
          referralEligibilityGrantModeRecharge: '充值后获得',
          referralEligibilityRechargeMinAmount: '最低累计充值额',
          referralEligibilityRechargeMinAmountHint: '充值后获得模式下，邀请人累计充值额（total_recharged）达到该金额才开启资格，与是否领券无关。0 表示只要有过任意充值即可。',
          referralInviterAmount: '邀请人达标奖励',
          referralInviterAmountHint: '被邀请人消费达标后，邀请人获得的赠金金额（USD）。',
          referralInviterGiftMode: '邀请人奖励扣除模式',
          referralInviterGiftModeHint: '控制邀请人达标赠金发放后的消耗方式。被邀请人注册赠金始终为优先扣除。',
          referralInviterGiftModePriority: '优先扣除',
          referralInviterGiftModeRatio: '比例扣除',
          referralInviterGiftRatio: '邀请人奖励扣除比例',
          referralInviterGiftRatioHint: '仅比例扣除模式生效。每消耗 $1 充值余额时，同步消耗的赠金余额。',
          referralInviterExpiryDays: '邀请人赠金有效期',
          referralInviterExpiryDaysHint: '邀请人奖励赠金的过期天数（从发放时刻起算）。',
          referralDiscountValidDays: '继承折扣有效期',
          referralDiscountValidDaysHint: '被邀请人从邀请人处继承充值折扣的有效天数。',
          referralRewardQuotaEnabled: '限制邀请人达标奖励发放次数',
          referralRewardQuotaEnabledHint: '开启后，邀请人需通过充值获得"领取被邀请人达标奖励"的机会；机会用尽后被邀请人达标也不再发奖，充值补足后立即补发。关闭则不限次数（当前行为）。',
          referralRewardQuotaRechargeStep: '每档充值额',
          referralRewardQuotaRechargeStepHint: '邀请人每充值该金额获得一档发放机会（未凑满一档的余额跨次累积）。',
          referralRewardQuotaPerBatch: '每档机会数',
          referralRewardQuotaPerBatchHint: '每充满一档充值额，邀请人获得的达标奖励发放机会数。',
          enabled: '启用邀请返利',
      ...
      registration: {
        ...
        emailSuffixWhitelistInputHint:
          "留空则不限制。使用 *.edu.cn 可匹配 edu.cn 及其子域名。高级：使用 re:<正则>#<说明> 按正则匹配整个邮箱，正则需以 ^ 开头、$ 结尾，例如 re:^\\d+{'@'}qq\\.com$#仅限纯数字QQ邮箱（按 Enter 添加）。",
        promoCode: '优惠码',
      ...
      webSearchEmulation: {
        ...
        apiKeyConfigured: '已配置',
        endpoint: 'API 地址',
        endpointHint: '留空则使用官方默认地址',
        endpointInvalid: 'API 地址必须是合法的 http(s) URL',
        showApiKey: '显示',
        hideApiKey: '隐藏',
        copyApiKey: '复制',
        copied: '已复制',
        ...
        healthUnhealthy: '异常',
        healthChecking: '待检测',
        healthStatusCode: '状态码',
        healthLatency: '延迟',
        healthCheckedAt: '检测时间',
        healthError: '错误',
        healthOnlyCustom: '仅对自定义 API 地址进行健康检查',
      },
      site: {
        title: '站点设置',
        description: '自定义站点品牌',
      ...
      payment: {
        ...
        easypayCustomType: '自定',
        easypayCustomTypeHint: '让易支付聚合到自定义通道：在网关侧用任意 type 字符串发起下单（如 epay / qqpay / usdt）。仅支持跳转/弹窗，不支持站内二维码。',
        easypayCustomTypeLabel: '通道 type',
        easypayCustomLabel: '按钮文案',
        easypayCustomIconUrl: '按钮图标 URL',
        easypayCustomMultiplier: '余额充值倍率覆盖',
        easypayCustomPrefix: '商品名前缀',
        easypayCustomSuffix: '商品名后缀',
        easypayCustomTypeRequired: '请填写自定通道的 type 字符串',
        easypayCustomTypeInvalid: 'type 仅允许字母、数字与下划线',
        easypayCustomTypeReserved: 'type 不能与 alipay/wxpay 等系统保留值重复',
        easypayCustomLabelRequired: '请填写按钮文案',
        easypayCustomIconUrlRequired: '请填写图标 URL',
        easypayCustomIconUrlInvalid: '图标 URL 必须以 http:// 或 https:// 开头',
        easypayCustomMultiplierInvalid: '倍率必须是大于 0 的数字',
      },
      balanceNotify: {
        title: '余额不足提醒',
