...
// gift_bonus 仅存在于完整 PaymentOrder（轻量状态轮询变体无此字段），用 hasAmountFields 收窄。
const giftBonus = computed(() =>
  hasAmountFields(order.value) ? resolveGiftBonusDisplay(order.value.gift_bonus) : null
)
