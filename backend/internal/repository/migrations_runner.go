...
const schedulerOutboxPendingDedupKeyMigration = "163_scheduler_outbox_pending_dedup_key_index_notx.sql"
const schedulerOutboxPendingDedupKeyIndex = "idx_scheduler_outbox_pending_dedup_key"
const latestAPIKeyIPIndexMigration = "191_add_usage_logs_api_key_latest_ip_index_notx.sql"
const latestAPIKeyIPIndex = "idx_usage_logs_api_key_latest_ip"
const usageLogsUpstreamModelMismatchIndexMigration = "214_add_usage_log_upstream_model_mismatch_index_notx.sql"
...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	// Upstream 195 (renumbered to 222 here) originally seeded mode=v2; flipped to v1
	// (safe default / opt-in v2). Existing DBs
	...
	for _, name := range files {
		...
		if nonTx {
			...
			for i, stmt := range statements {
				...
				// 使用 lockConn 保证非事务语句与迁移持锁在同一连接上执行。
