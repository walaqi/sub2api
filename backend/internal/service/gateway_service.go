...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthrough(
	...
	// 账号级请求头覆写（最终生效，覆盖上面所有来源的同名头，含 origin model-id）
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID, originModelHeaderValue string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	// 放在所有 header 逻辑之后，确保配置值对同名头拥有最终决定权（含 origin model-id）。
	account.ApplyHeaderOverrides(req.Header)
...
type RecordUsageInput struct {
	...
	MetadataUserID     string             // 客户端自报 metadata.user_id 原文；仅用于解析 device_id 做滥用检测。传标量而非整个 ParsedRequest，避免 worker 池保活大请求体。
...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	...
	if input.MetadataUserID != "" {
		if uid := ParseMetadataUserID(input.MetadataUserID); uid != nil {
...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() { //nolint:gosec // path is from trusted env config, not user input
		path = filepath.Join(path, debugGatewayBodyDefaultFilename)
	}

	// 确保父目录存在
	if dir := filepath.Dir(path); dir != "." {
		if err := os.MkdirAll(dir, 0755); err != nil { //nolint:gosec // path is from trusted env config
			slog.Error("failed to create gateway debug log directory", "dir", dir, "error", err)
			return
		}
	}

	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644) //nolint:gosec // path is from trusted env config
	if err != nil {
