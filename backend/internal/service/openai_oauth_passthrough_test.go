...
func TestOpenAIGatewayService_OAuthPassthrough_OfficialIdentityUnified(t *testing.T) {
	...
	require.Equal(t, openai.CodexDefaultOriginator, upstream.lastReq.Header.Get("originator"))
	require.Equal(t, codexCLIVersion, upstream.lastReq.Header.Get("version"))
	require.Empty(t, upstream.lastReq.Header.Get("x-codex-installation-id"), "fingerprint convergence is opt-in when no seed is configured")
	require.Empty(t, upstream.lastReq.Header.Get("session-id"))
	require.Empty(t, upstream.lastReq.Header.Get("thread-id"))
	require.Empty(t, gjson.GetBytes(upstream.lastBody, "client_metadata.session_id").String())
	require.Empty(t, gjson.GetBytes(upstream.lastBody, "client_metadata.thread_id").String())
