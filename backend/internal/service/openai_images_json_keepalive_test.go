...
// fork 语义锁定：OAuth 非流式图片走 native Responses API passthrough（01e8815d
// 为 image-studio 引入），返回 {"object":"response","output":[...]}，而非旧版
// images-API 的 {"data":[{"b64_json":...}]}；图片数从 response.output_item.done
// 事件统计。此测试断言 JSON keepalive 心跳与该 passthrough 输出并存且不破坏其合法性。
func TestOpenAIImagesJSONKeepalive_KeepsOAuthNonStreamResponseValid(t *testing.T) {
	...
	go func() {
		...
		_, _ = io.WriteString(writer,
			"data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"image_generation_call\",\"result\":\"aW1hZ2U=\",\"output_format\":\"png\"}}\n\n"+
				"data: {\"type\":\"response.completed\",\"response\":{\"object\":\"response\",\"created_at\":1710000000,\"output\":[]}}\n\n"+
				"data: [DONE]\n\n",
		)
		_ = writer.Close()
	}()
	...
	_, imageCount, _, err := svc.handleOpenAIImagesOAuthNonStreamingResponse(resp, c, "gpt-image-2")
	stop()
	...
	require.True(t, json.Valid(rec.Body.Bytes()), rec.Body.String())
	// fork passthrough 格式：response 对象 + output 数组，图片在 output.0.result。
...
//
// fork 语义：OAuth 图片路径的 failover 由上游 HTTP 状态码判定（≥500 等，见
// shouldFailoverOpenAIUpstreamResponse），而非 200-OK SSE body 里的 error 事件——
// 后者的 OAuth failover 在 1572c9b91 已刻意 drop（fork 走 image-studio Responses
// passthrough，不解析 SSE body 做换号）。故此处用 HTTP 502 触发 fork 支持的 failover，
// 验证心跳空白字节不会阻断该判定。
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Body: io.NopCloser(strings.NewReader(
					`{"error":{"type":"server_error","code":"server_error","message":"The image service is temporarily unavailable."}}`,
				)),
			},
		},
	}
