...
func defaultImageDownloadHTTPClient() *http.Client {
	...
	if err != nil {
		// No proxy or custom TLS options are used here, so construction cannot
		// normally fail. Keep a fail-closed transport if that assumption changes.
		return &http.Client{Transport: imageDownloadRejectTransport{err: err}}
	}
	return client
}

type imageDownloadRejectTransport struct{ err error }

func (t imageDownloadRejectTransport) RoundTrip(*http.Request) (*http.Response, error) {
	return nil, t.err
}
...
func (u *ImageResultUploader) download(ctx context.Context, rawURL string) ([]byte, string, error) {
	...
	// The default client validates resolved IPs on every new host, including
	// redirect targets. Tests may inject an isolated client explicitly.
	resp, err := u.httpClient.Do(req) // #nosec G704 -- default transport blocks private/reserved resolved IPs and DNS rebinding
	if err != nil {
