...
func TestOpenAIImagesOAuthBodyReadTransportErrorFailover(t *testing.T) {
	...
	_, _, _, readErr := svc.handleOpenAIImagesOAuthNonStreamingResponse(resp, c, "gpt-image-2")
...
func TestOpenAIGatewayServiceForwardImagesOAuthStreamReadErrorBeforeWriteReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	svc := &OpenAIGatewayService{httpUpstream: &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"X-Request-Id": []string{"req_stream_read_failure"}},
		Body:       &openAIImagesReadErrorBody{err: errors.New("unexpected EOF")},
	}}}
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	require.NoError(t, err)
	account := &Account{
		ID: 5402, Name: "openai-oauth-stream", Platform: PlatformOpenAI, Type: AccountTypeOAuth,
...
func TestOpenAIGatewayServiceForwardImages_OAuthStreamingTransformsEvents(t *testing.T) {
	...
	require.True(t, result.Stream)
	require.Equal(t, 1, result.ImageCount)
	// #4402: billing now captures tool_usage.image_gen from response.completed.
