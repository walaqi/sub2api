...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	tests := []struct {
		...
		// GPT-5.5 Pro 自 #5851 起有独立本地兜底定价（官方价）。运行时仍优先用线上 model-pricing 数据。
		{name: "openai gpt5.5 pro dedicated fallback", model: "gpt-5.5-pro", expectedInput: 30e-6, expectedOutput: floatPtr(180e-6), expectedCacheRead: floatPtr(30e-6)},
