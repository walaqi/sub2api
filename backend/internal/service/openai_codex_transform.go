...
func normalizeOpenAIModelForUpstreamWithPolicy(account *Account, model string, explicitlyMapped bool) string {
	// Upstream #5888 broadened the OAuth check from Type==OAuth to
	// UsesOpenAICodexProtocol() (also covers OpenAI OAuth-like / SetupToken
	// accounts); keep the fork's explicitlyMapped policy so admin-mapped
	// passthrough models are not collapsed to gpt-5.4.
