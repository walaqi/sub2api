...
func TestOpenAIGatewayServiceForwardImages_OAuthUsesDecodedOutputDimensions(t *testing.T) {
	...
	// fork（seq263，01e8815d）非流式 OAuth 图片走 Responses passthrough，客户端收到的是
	// Responses 结构（output[].type=image_generation_call），而非扁平的 images 结构，
	// 因此不再断言顶层 size/quality。
	body := run.recorder.Body.String()
	require.Equal(t, "image_generation_call", gjson.Get(body, "output.0.type").String())
	require.NotEmpty(t, gjson.Get(body, "output.0.result").String())

	// seq274 核心：计费尺寸来自解码后的真实图片字节（1672x941），而非上游报告的 "auto"。
...
func TestOpenAIGatewayServiceForwardImages_OAuthStreamingUsesDecodedOutputDimensions(t *testing.T) {
	...
	// fork（seq263）流式 OAuth 图片原样透传上游 SSE（passthrough），客户端收到
	// response.output_item.done 而非合成的 image_generation.completed 事件；透传的
	// data 行不带 event: 前缀，因此按 JSON type 匹配。
	events := parseOpenAIImageTestSSEEvents(run.recorder.Body.String())
	var doneData string
	for _, e := range events {
		if gjson.Get(e.Data, "type").String() == "response.output_item.done" {
			doneData = e.Data
			break
		}
	}
	require.NotEmpty(t, doneData, "应透传 response.output_item.done 事件")
	require.Equal(t, "image_generation_call", gjson.Get(doneData, "item.type").String())
	require.NotEmpty(t, gjson.Get(doneData, "item.result").String())

	// seq274 核心：计费尺寸来自解码后的真实图片字节（1672x941）。
	require.Equal(t, []string{"1672x941"}, run.result.ImageOutputSizes)
}
...
func runOpenAIOAuthImageActualSizeTest(t *testing.T, stream bool) openAIOAuthImageActualSizeTestRun {
	...
	// fork（seq263）的 OAuth 图片路径从 response.output_item.done 事件提取图片
	// （非流式与流式都是如此），因此 fixture 必须携带该事件，seq274 的解码尺寸
	// 逻辑才会命中。上游报告 size="auto"，解码后真实尺寸为 1672x941。
	upstreamBody := fmt.Sprintf(
		"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000000,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"size\":\"auto\",\"quality\":\"auto\",\"output_format\":\"png\"}]}}\n\n"+
			"data: {\"type\":\"response.output_item.done\",\"item\":{\"id\":\"ig_actual_size\",\"type\":\"image_generation_call\",\"size\":\"auto\",\"quality\":\"auto\",\"output_format\":\"png\",\"result\":%q}}\n\n"+
