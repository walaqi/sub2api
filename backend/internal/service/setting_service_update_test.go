...
func TestSettingService_AffiliateAdminRechargeSetting(t *testing.T) {
	...
	t.Run("value is persisted", func(t *testing.T) {
		...
		// Fork requires a non-empty DefaultSubscriptions list (DEFAULT_SUBSCRIPTIONS_EMPTY guard);
		// supply a valid one so UpdateSettings reaches the affiliate-recharge persistence path.
...
func TestSettingService_UpdateSettings_OpenAIAdvancedSchedulerWeightSums(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			weights: SystemSettings{
				...
				// Fork requires a non-empty DefaultSubscriptions list (DEFAULT_SUBSCRIPTIONS_EMPTY
				// guard); supply a valid one so this success case reaches the weight-sum path.
				DefaultSubscriptions: []DefaultSubscriptionSetting{
					{GroupID: 11, ValidityDays: 30},
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Fork's DEFAULT_SUBSCRIPTIONS_EMPTY guard requires a group reader for the
			// success path that supplies DefaultSubscriptions; harmless for error cases.
			svc.SetDefaultSubscriptionGroupReader(&defaultSubGroupReaderStub{
				byID: map[int64]*Group{
					11: {ID: 11, SubscriptionType: SubscriptionTypeSubscription},
				},
			})
