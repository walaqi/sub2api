...
func TestAuthServiceVerifyActionCaptchaIfEnabledBypassesAliyunForTrustedClient(t *testing.T) {
	spy := &aliyunVerifierSpy{}
	authService := newAliyunAuthServiceForTest(&config.Config{}, aliyunEnabledSettings(), spy)

	err := authService.VerifyActionCaptchaIfEnabled(
		ContextWithTurnstileBypass(context.Background()),
		CaptchaProof{},
		"127.0.0.1",
	)
