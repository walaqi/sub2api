...
func newImageCredService(repo APIKeyRepository) *ImageStudioService {
	...
	cfg.ImageStudio.ImageGroupID = 7
	return &ImageStudioService{cfg: cfg, apiKeyRepo: repo}
}

// imageOKGroup 返回一个满足出图门槛的 group。
func imageOKGroup(id int64) *Group {
	return &Group{ID: id, Name: "图片专用", Platform: PlatformOpenAI, AllowImageGeneration: true}
}
...
func TestListImageCapableKeysNoImageGroupConfigured(t *testing.T) {
	repo := &imageCredRepoStub{byUser: map[int64][]APIKey{1: {}}}
	svc := newImageCredService(repo)
	svc.cfg.ImageStudio.ImageGroupID = 0 // 未配置图片专用 group
	list, err := svc.ListImageCapableKeys(context.Background(), 1)
	if err != nil {
		t.Fatalf("ListImageCapableKeys: %v", err)
	}
	if list.CanCreate {
		t.Fatalf("expected CanCreate=false when no image group configured")
	}
}
