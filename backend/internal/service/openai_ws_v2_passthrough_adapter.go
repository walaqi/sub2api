...
// account's ordinary model_mapping and Codex aliases must not rewrite it — this
// value feeds the fast-policy whitelist decision, and upstream #5888 aligned it
// with the HTTP passthrough path (which also keeps the raw/channel-mapped model
// rather than applying account model_mapping).
