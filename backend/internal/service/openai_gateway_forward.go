...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	// 账号映射命中或渠道映射（handler 写入 body + context 标记）均视为管理员显式配置。
	_, accountExplicitlyMapped := account.ResolveMappedModel(requestedModel)
	explicitlyMapped := accountExplicitlyMapped || OpenAIChannelModelMappedFromContext(ctx)
	// 采用 upstream #5888 的共享映射链（修复 compact 映射须按客户端模型优先于普通
	// 账号映射的既有 fork bug）。
	...
	if isCompactRequest {
		...
		// fork 计费特性：管理员显式映射的未知 gpt-5* 目标不被兜底折叠为 gpt-5.4，
		// 直通到上游（如 gpt-5.6-sol）。已知模型在两种策略下结果一致，故仅在显式
		// 映射且非 compact 时覆盖 upstreamModel。
		if policyModel := normalizeOpenAIModelForUpstreamWithPolicy(account, billingModel, true); policyModel != "" {
	...
	for {
		...
		if resp.StatusCode >= 400 {
			...
			openAIImageDumpBytes(c, resp, respBody, "responses_4xx")
		...
		openAIImageDumpTeeBody(c, resp, "responses_2xx")
