...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	if err := h.billingCacheService.CheckBillingEligibility(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(c.Request.Context(), apiKey), model); err != nil {
...
func (h *OpenAIGatewayHandler) GrokVoice(c *gin.Context, endpoint string) {
	...
	requestModel := extractGrokVoiceModel(body)
	if requestModel == "" {
		requestModel = selectionModel
	}
	subscription, _ := middleware2.GetSubscriptionFromContext(c)
	...
	if endpoint == "tts" {
		...
		if decision := h.checkContentModeration(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIChat, requestModel, auditBody); decision != nil && decision.Blocked {
...
func extractGrokVoiceModel(body []byte) string {
	if len(body) == 0 {
		return ""
	}
	var payload struct {
