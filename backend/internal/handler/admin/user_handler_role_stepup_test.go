...
func doJSONPayload(t *testing.T, router *gin.Engine, method, path string, payload map[string]any) *httptest.ResponseRecorder {
	t.Helper()
...
func TestUpdateUserPromoteToAdminRequiresStepUp(t *testing.T) {
	...
	rec := doJSONPayload(t, router, http.MethodPut, "/api/v1/admin/users/1", map[string]any{"role": "admin"})
...
func TestUpdateUserKeepAdminRoleSkipsStepUp(t *testing.T) {
	...
	rec := doJSONPayload(t, router, http.MethodPut, "/api/v1/admin/users/2", map[string]any{"role": "admin"})
...
func TestUpdateUserRegularRoleSkipsStepUp(t *testing.T) {
	...
	rec := doJSONPayload(t, router, http.MethodPut, "/api/v1/admin/users/1", map[string]any{"role": "user", "email": "u@example.com"})
	require.Equal(t, http.StatusOK, rec.Code)
}
...
func TestCreateAdminUserRequiresStepUp(t *testing.T) {
	...
	rec := doJSONPayload(t, router, http.MethodPost, "/api/v1/admin/users", map[string]any{
