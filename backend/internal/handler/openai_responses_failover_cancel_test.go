...
// openAIImagesFailoverAccountRepo 是一个最小 AccountRepository mock。
// 上游 #3155（fix/openai-images-server-error-failover）在 openai_images_failover_test.go
// 中定义了它，但该 PR 早于本轮 merge 起点且从未合入 fork；seq270 的取消感知
// failover 测试复用了这个类型，因此在此就地补齐定义。
