...
func newKeyBillingRouteTestRouter(runMode string) (*gin.Engine, *keyBillingRouteRateRepo, string) {
	...
	// fork 定制守卫：非 simple 模式下 NewGatewayService 要求 giftEngine 非 nil（构造期
	// fail-fast）。key-billing 是只读自省接口，不走余额扣费兜底路径，giftEngine 保持 nil 无碍。
	// 临时用 simple 模式构造以合法通过守卫，构造后还原 runMode（cfg 是共享指针，请求时读到还原值）。
	prevRunMode := cfg.RunMode
