...
func TestMigration193DefaultsOrdinaryOpenAIAndInheritsForSparkShadows(t *testing.T) {
	content, err := FS.ReadFile("193_default_openai_long_context_billing.sql")
...
func TestMigration193GuardsMixedVersionAccountWrites(t *testing.T) {
	content, err := FS.ReadFile("193_default_openai_long_context_billing.sql")
	require.NoError(t, err)
