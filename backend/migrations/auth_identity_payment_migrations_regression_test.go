...
func TestMigration161AddsAccountAutoPauseExpiryPartialIndex(t *testing.T) {
	content, err := FS.ReadFile("161_account_autopause_expiry_index_notx.sql")
	require.NoError(t, err)
...
func TestMigration185BackfillsGrokMediaGenerationGroups(t *testing.T) {
	content, err := FS.ReadFile("185_enable_grok_media_generation_groups.sql")
	require.NoError(t, err)
...
func TestMigration183AddsSparkShadowColumnsAndConstraintsWithoutHotIndexes(t *testing.T) {
	content, err := FS.ReadFile("183_account_spark_shadow.sql")
	require.NoError(t, err)
...
func TestMigration183aAddsSparkShadowIndexesConcurrently(t *testing.T) {
	content, err := FS.ReadFile("183a_account_spark_shadow_indexes_notx.sql")
	require.NoError(t, err)
...
func TestMigration189AllowsCyberBlockedUsageRequestType(t *testing.T) {
	...
	for i, entry := range entries {
		switch entry.Name() {
		case "188_video_per_second_billing_metadata.sql":
			previousIndex = i
		case "189_allow_cyber_blocked_usage_request_type.sql":
			currentIndex = i
		}
	}
	...
	content, err := FS.ReadFile("189_allow_cyber_blocked_usage_request_type.sql")
	require.NoError(t, err)
