...
- Next index: none
- Next commit: none
- Last action: merged high-risk #248 as `afe4c7d50`; all 9 incremental first-parent units are resolved and complete gates passed with documented unit/lint baselines and shared-DB integration isolation.
...
- #248 `1d3b9665c`: merged as `afe4c7d50`. Native remote compaction v2 is recognized by the streaming `compaction_trigger` body signal and stays on the Responses protocol; legacy `/responses/compact` retains compact capability and model mapping. Account scheduling, channel restriction and profitability checks use the model chain actually forwarded for OAuth, passthrough and raw Chat Completions fallback accounts. Main's composed content moderation/prompt-audit remains before routing, and request fingerprint, RecordUsage, gift allocation and super-invite tracking are unchanged. Focused tests and ordinary Go passed; frontend ESLint/typecheck and 1574/1574 tests passed. Unit retained only the known three config reachability keys and CheckerNil baseline. Full integration had only the known cross-package keybind shared-database interference and the complete keybind package passed independently. Lint retained exactly the three known G704 findings.
